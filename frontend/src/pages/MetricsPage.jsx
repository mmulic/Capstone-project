...
const MetricsPage = () => {
  ...
  useEffect(() => {
    // Try ML/Supabase stats first; fall back to local PostgreSQL stats
    getMLStats()
      .then(setStats)
      .catch(() => {
        api.get("/api/stats")
          .then(({ data }) => {
            const dist = data.damage_distribution ?? {};
            // Normalise local stats into the same shape MetricsPage expects
            setStats({
              by_disaster: { "dataset": dist },
              overall_distribution: dist,
              total_predictions: data.total_assessed ?? 0,
              disasters_count: 1,
            });
          })
  ...
  const distData = (() => {
    ...
    // Support both hyphen (Supabase) and underscore (local DB) key formats
    const lookup = (bucket, key) =>
      bucket?.[key] ?? bucket?.[key.replace(/-/g, "_")] ?? bucket?.[key.replace(/_/g, "-")] ?? 0;

    return ["no-damage", "minor-damage", "major-damage", "destroyed"].map((key) => {
      const row = { category: key.replace(/-/g, " ").toUpperCase() };
      disasters.forEach((d) => { row[d] = lookup(byDisaster[d], key); });
      return row;
    });
  })();
