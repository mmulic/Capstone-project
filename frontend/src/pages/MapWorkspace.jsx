...
// Flies the map to a specific lat/lng point (used by chatbot map_focus).
function FlyToPoint({ target }) {
  const map = useMap();
  useEffect(() => {
    if (target) map.flyTo([target.lat, target.lng], target.zoom ?? 15, { animate: true, duration: 1.2 });
  }, [target, map]);
  return null;
}
...
function GeoMapWorkspace({ sceneOptions, basemap }) {
  const { imageryLayer, setImageryLayer, harveyData, selectedSceneId, setSelectedSceneId, mapFocusTarget } = useAppContext();
    ...
    <div className="flex-1 flex flex-col overflow-hidden relative">
      ...
      <div className="flex-1 relative">
        <MapContainer center={MAP_CENTER} zoom={MAP_ZOOM} className="w-full h-full" zoomControl>
          ...
          {/* Fly to chatbot-requested location */}
          {mapFocusTarget && <FlyToPoint target={mapFocusTarget} />}
...
function AIAssistantTab() {
  const { chatHistory, setChatHistory, setMapFocusTarget, setRightTab } = useAppContext();
  ...
  const send = async () => {
    ...
    try {
      // Pass full conversation history so backend maintains context
      const history = [...chatHistory, userMsg].map((m) => ({ role: m.role, text: m.text }));
      const res = await postQuery(text, history);
      setChatHistory((h) => [...h, { role: "assistant", text: res.response || res.answer || res.message || "No response.", id: Date.now() + 1 }]);

      // If backend identified a geographic focus location, fly the map there
      if (res.map_focus) {
        setMapFocusTarget(res.map_focus);
        // Switch to map view so the user sees the zoom
        setRightTab("details");
      }
    } catch {
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {chatHistory.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-xs text-gray-400 mt-1 leading-relaxed">Ask about damage levels, FEMA info, or a specific location — I'll focus the map for you.</p>
          </div>
        ) : (
          chatHistory.map((msg) => (
